...
#Read tweeter data ####
json_small <- jsonlite::fromJSON('twitter clustering/exp2/clusterInput_big.json',flatten=T) # a list in which each element is one of your original JSON files
...
bad.vs <- V(dataset_small.network)[degree(dataset_small.network)<3] #identify those vertices part of less than X edges
...
cluster_result_itzik <- jsonlite::fromJSON('twitter clustering/exp2/clusterOutput_big.json',flatten=T) # a list in which each element is one of your original JSON files

write(jsonlite::toJSON(louvain_clust_dataset_small),"myOutput_big.json") #json 

#Statistics
summary(as.numeric(table(louvain_clust_dataset_small$X4))); sd(as.numeric(table(louvain_clust_dataset_small$X4)))
summary(as.numeric(table(cluster_result_itzik$X4))); sd(as.numeric(table(cluster_result_itzik$X4)))
